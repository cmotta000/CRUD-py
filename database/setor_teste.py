...
def sql():
   dao = SetorDAO()
   objs = dao.execute_sql_and_fetch('select nme_setor, sum(vlr_servico) as soma from tt_setor join tb_servico on idt_setor=cod_setor where idt_setor=:idt_setor group by nme_setor', {'idt_setor' : '1'})
   print("Dados da consulta:")
   for obj in objs:
       print(obj)
